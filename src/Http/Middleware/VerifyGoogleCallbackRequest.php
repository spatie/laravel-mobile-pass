...
class VerifyGoogleCallbackRequest
{
    ...
    private function verifyAndDecode(array $payload, string $issuerId): array
    {
        ...
        if (is_array($cachedKeys) && $this->extractUsableKeyValues($cachedKeys) !== []) {
            try {
                ...
                // Cached keys did not verify the payload. This may be a key
                // rotation or simply a forged payload. Refetch once more, but
                // do not bust the cache — busting on every signature failure
                // would let an attacker pin our upstream by spamming bad
                // payloads. The fresh fetch below either rotates the cached
                // keys or fails the second verify, which is the right outcome.
            }
        }
    ...
    private function resolveCacheTtlSeconds(array $keys): int
    {
        ...
        $ttl = intdiv($earliestExpiration - $now, 1000) - self::ROOT_KEYS_TTL_SAFETY_MARGIN_SECONDS;

        return max(self::ROOT_KEYS_MIN_TTL_SECONDS, $ttl);
    }
