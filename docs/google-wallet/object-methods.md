...
Every Google builder extends `GooglePassBuilder`, which handles the bits of a pass that are specific to one user. Look-and-feel (logos, colours, event name, venue) lives on the Class, covered in [Pass classes](/docs/laravel-mobile-pass/v1/google-wallet/pass-classes).
