...
If you've opted into the queue (see the [Updating a pass](basic-usage/updating-a-pass) page for the setup), remember to call `Queue::fake()` in your tests so update pushes don't actually ship. Assertions then look like:
